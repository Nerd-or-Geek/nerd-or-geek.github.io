...
async function pushToGithub() {
    ...
    try {
        const fileResponse = await fetch(`https://api.github.com/repos/${settings.owner}/${settings.repo}/contents/data/site-data.json`, {
            headers: {
                'Authorization': `token ${settings.token}`,
                'Accept': 'application/vnd.github.v3+json'
            }
        ...
        const updateResponse = await fetch(`https://api.github.com/repos/${settings.owner}/${settings.repo}/contents/data/site-data.json`, {
            method: 'PUT',
            headers: {
                'Authorization': `token ${settings.token}`,
                'Accept': 'application/vnd.github.v3+json',
...
function setupThemeSelectors() {
    document.querySelectorAll('.theme-preset').forEach(preset => {
        preset.addEventListener('click', () => {
            ...
            const themeInput = document.getElementById('projectThemePreset');
            if (themeInput && theme) {
...
function openProjectModal(project) {
    ...
    document.getElementById('projectThemePreset').value = 'default';
    ...
    if (project) {
        ...
        if (project.theme) {
            ...
            document.getElementById('projectThemePreset').value = themePreset;
            if (themePreset === 'custom') {
                document.getElementById('customThemeColors').style.display = 'block';
                document.getElementById('themePrimaryColor').value = project.theme.primaryColor || '#0ea5e9';
                document.getElementById('themeAccentColor').value = project.theme.accentColor || '#38bdf8';
                document.getElementById('themeTextColor').value = project.theme.textColor || '#f8fafc';
                document.getElementById('themeCardBgColor').value = project.theme.cardBgColor || '#1e293b';
                document.getElementById('themeHeroBgColor').value = project.theme.heroBgColor || '#0f172a';
            }
        }
    }
    else {
...
function saveProject(e) {
    ...
    const themePreset = document.getElementById('projectThemePreset').value;
    const theme = { preset: themePreset };
    if (themePreset === 'custom') {
        theme.primaryColor = document.getElementById('themePrimaryColor').value;
        theme.accentColor = document.getElementById('themeAccentColor').value;
        theme.textColor = document.getElementById('themeTextColor').value;
        theme.cardBgColor = document.getElementById('themeCardBgColor').value;
        theme.heroBgColor = document.getElementById('themeHeroBgColor').value;
    }
